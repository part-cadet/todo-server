...
exports.getAllMembersOfBoard = (request, response, params) => {
  const validToken = auth.hasAccess(request)
  // console.log(validToken)
  ...
  if (validToken.verify) {
    ...
    const validationError = ajv.errors
    if (valid) {
      db.getAllMembersOfBoard(params.taskID)
        .then((result) => {
          const responseBody = {
            status: 'Ok',
            result: result
  ...
  if (validToken.verify) {
    getRawBody(request)
      .then((bodyBuffer) => {
        ...
        const validationError = ajv.errors
        if (valid) {
          db.addBoard(body, validToken.payload.username)
            .then((result) => {
              const responseBody = {
                status: 'Ok',
    ...
    getRawBody(request)
      .then((bodyBuffer) => {
        ...
        if (idValid) {
          ...
          if (bodyValid) {
            db.updateBoard(params.boardID, body, validToken.payload.username)
              .then((result) => {
                if (result > 0) {
                  const responseBody = {
                    status: 'Ok',
                  ...
                  const responseBody = {
                    ...
                    message: `Only the owner of the board can change the board's info.`
                  }
                  errorNotFound(response, responseBody)
                }
              })
...
exports.removeBoard = (request, response, params) => {
  ...
  if (validToken.verify) {
    ...
    const validationError = ajv.errors
    if (valid) {
      db.removeBoard(params.boardID, validToken.payload.username)
        .then((result) => {
          if (result > 0) {
            const responseBody = {
              status: 'Ok',
            ...
            const responseBody = {
              status: 'Access Denied',
              message: 'Only the owner of the board can delete the board'
            }
            errorNotFound(response, responseBody)
          }
        })
            ...
            db.updateTask(params.taskID, body)
              .then((result) => {
                if (result > 0) {
                  ...
                  ok(response, responseBody)
                } else if (typeof body.assignee !== 'undefined') {
                  const responseBody = {
                    status: 'Invalid Member',
                    message: 'Entered user is not a member of this board.'
                  }
...
exports.removeCurrentUser = (request, response) => {
  const validToken = auth.hasAccess(request)
  if (validToken.verify) {
    db.removeCurrentUser(validToken.payload.username)
      .then((result) => {
        if (result > 0) {
          const responseBody = {
            status: 'Ok',
...
exports.removeMember = (request, response, params) => {
  ...
  if (validToken.verify) {
    ...
    const validationError = ajv.errors
    if (valid) {
      db.removeMember(params, validToken.payload.username)
        .then((result) => {
          if (result > 0) {
            const responseBody = {
              status: 'Ok',
            ...
            const responseBody = {
              status: 'Access Denied',
              message: 'Only the owner of the board can delete members.'
            }
            errorNotFound(response, responseBody)
          }
        })
