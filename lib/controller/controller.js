  ...
  getRawBody(request)
    .then((bodyBuffer) => {
      ...
      if (validID) {
        ...
        const bodyvalidationError = ajv.errors
        if (validBody) {
          db.addTask(body.description, body.done, body.assignee, params.todoID)
            .then((result) => {
              console.log(result)
              const responseBody = {
                status: 'Ok',
...
exports.updateTask = (request, response, params) => {
  getRawBody(request)
    .then((bodyBuffer) => {
      ...
      const idValidationError = ajv.errors
      if (idValid) {
        const bodyValid = ajv.validate(schema.taskUpdateSchema, body)
        const bodyValidationError = ajv.errors

        if (bodyValid) {
          db.updateBoard(params.taskID, body)
            .then((result) => {
              if (result > 0) {
                const responseBody = {
                  status: 'Ok',
